...
class APIs {
  ...
  static userDiaryList(page, pageSize, uId) {
    return DOMAIN + `flm/diaryList/${page}/${pageSize}/${uId}`;
  }

  //일기 하나 삭제하기.
  static filmDiaryDelete = DOMAIN + 'flm/diaryDelete';

  static filmDiarySearch = DOMAIN + 'flm/diarySearch';
}
...
class NetTool {
  ...
  exec = (isProgressUI = false) =>
    new Promise((resolve, reject) => {
      ...
      let opt;
      if (this.mFormData == null) {
        opt = { method: 'get', url: url };
      } else {
        opt = { method: 'post', url: url, data: this.mFormData };
      }
